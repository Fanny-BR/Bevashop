...
<?php
    ...
    if(isset($_POST['filter']) && ! empty($_POST['filter'])){ // Cek apakah user telah memilih filter dan klik tombol tampilkan
        ...
        if($filter == '1'){ // Jika filter nya 1 (per tanggal)
            ...
            JOIN barang ON stok.id_barang = barang.id  WHERE DATE(tgl_transaksi)='".$_POST['tanggal']."'"; // Tampilkan data transaksi sesuai tanggal yang diinput oleh user pada filter
            ...
            JOIN transaksi ON detail_transaksi.id_transaksi = transaksi.id
            JOIN stok ON detail_transaksi.id_stok = stok.id 
            JOIN barang ON stok.id_barang = barang.id  WHERE MONTH(tgl_transaksi)='".$_POST['bulan']."' AND YEAR(tgl_transaksi)='".$_POST['tahun']."'"; // Tampilkan data transaksi sesuai bulan dan tahun yang diinput oleh user pada filter
            ...
            JOIN transaksi ON detail_transaksi.id_transaksi = transaksi.id
            JOIN stok ON detail_transaksi.id_stok = stok.id 
            JOIN barang ON stok.id_barang = barang.id  WHERE YEAR(tgl_transaksi)='".$_POST['tahun']."'"; // Tampilkan data transaksi sesuai tahun yang diinput oleh user pada filter
        }else{
            ...
            JOIN transaksi ON detail_transaksi.id_transaksi = transaksi.id
            JOIN stok ON detail_transaksi.id_stok = stok.id 
            JOIN barang ON stok.id_barang = barang.id  WHERE DATE(tgl_transaksi) BETWEEN '".$_POST['tawal']."' AND '".$_POST['takhir']."'";
        }
    }else{ // Jika user tidak mengklik tombol tampilkan
        echo '<b>Semua Data Transaksi</b><br /><br />';
        echo '<a class="btn btn-danger" href="halaman_laporan/print.php"><i class="fa fa-print"></i> Cetak PDF</a>';
        echo '<a class="btn btn-warning pull-right" href="index.php?page=laporan"><i class="fa fa-refresh"></i> Reset Filter</a></br></br>';
        ...
        JOIN transaksi ON detail_transaksi.id_transaksi = transaksi.id
        JOIN stok ON detail_transaksi.id_stok = stok.id 
        JOIN barang ON stok.id_barang = barang.id "; // Tampilkan semua data transaksi diurutkan berdasarkan tanggal
    }
    ?>
    ...
    <div class="box-body">
        ...
        if(isset($_POST['filter']) && ! empty($_POST['filter'])){ // Cek apakah user telah memilih filter dan klik tombol tampilkan
            ...
            if($filter == '1'){ // Jika filter nya 1 (per tanggal)
                ...
                // $datein = format_indo(date('Y-m-d'));
                echo '<b>Data Transaksi Tanggal '.$tgl.'</b><br /><br />';
                $query2 = "SELECT hr.harga, SUM(hr.qty) AS qty, SUM(hr.total) AS total, hr.nama
                FROM (	SELECT transaksi.tgl_transaksi, detail_transaksi.harga, detail_transaksi.qty, detail_transaksi.total AS total, barang.nama
                    FROM detail_transaksi
                    JOIN transaksi ON detail_transaksi.id_transaksi = transaksi.id
                    JOIN stok ON detail_transaksi.id_stok = stok.id 
                    JOIN barang ON stok.id_barang = barang.id  
                    WHERE DATE(tgl_transaksi)='".$_POST['tanggal']."' 
                    ) hr
                GROUP BY `hr`.`nama`  ";
                $query2 = "SELECT transaksi.tgl_transaksi,transaksi.no_invoice, detail_transaksi.id_stok, barang.nama, detail_transaksi.harga, detail_transaksi.qty, detail_transaksi.total
                FROM detail_transaksi
                JOIN transaksi ON detail_transaksi.id_transaksi = transaksi.id
                JOIN stok ON detail_transaksi.id_stok = stok.id 
                JOIN barang ON stok.id_barang = barang.id  WHERE DATE(tgl_transaksi)='".$_POST['tanggal']."'"; // Tampilkan data transaksi sesuai tanggal yang diinput oleh user pada filter
                ...
                FROM (	SELECT transaksi.tgl_transaksi, detail_transaksi.harga, detail_transaksi.qty, detail_transaksi.total AS total, barang.nama
                    ...
                    JOIN stok ON detail_transaksi.id_stok = stok.id 
                    JOIN barang ON stok.id_barang = barang.id  
                    WHERE MONTH(transaksi.`tgl_transaksi`)='".$_POST['bulan']."' AND YEAR(transaksi.`tgl_transaksi`)='".$_POST['tahun']."'
                    ) hr
                GROUP BY `hr`.`nama`  "; // Tampilkan data transaksi sesuai bulan dan tahun yang diinput oleh user pada filter
            }else if($filter == '3'){ // Jika filter nya 3 (per tahun)
                echo '<b>Data Transaksi Tahun '.$_POST['tahun'].'</b><br /><br />';
                $query2 = "SELECT hr.harga, SUM(hr.qty) AS qty, SUM(hr.total) AS total, hr.nama
                FROM (	SELECT transaksi.tgl_transaksi, detail_transaksi.harga, detail_transaksi.qty, detail_transaksi.total AS total, barang.nama
                    ...
                    JOIN stok ON detail_transaksi.id_stok = stok.id 
                    JOIN barang ON stok.id_barang = barang.id  
                    WHERE YEAR(tgl_transaksi)='".$_POST['tahun']."'
                    ) hr
                GROUP BY `hr`.`nama`  ";// Tampilkan data transaksi sesuai tahun yang diinput oleh user pada filter
            }else{
                $tgl_awal = date('d-m-Y', strtotime($_POST['tawal']));
                $tgl_akhir = date('d-m-Y', strtotime($_POST['takhir']));
                echo '<b>Data Transaksi Tanggal '.$tgl_awal.' Sampai '.$tgl_akhir.'</b><br /><br />';
                ...
                FROM (	SELECT transaksi.tgl_transaksi, detail_transaksi.harga, detail_transaksi.qty, detail_transaksi.total AS total, barang.nama
                    ...
                    JOIN stok ON detail_transaksi.id_stok = stok.id 
                    JOIN barang ON stok.id_barang = barang.id  
                    WHERE DATE(tgl_transaksi) BETWEEN '".$_POST['tawal']."' AND '".$_POST['takhir']."'
                    ) hr
                GROUP BY `hr`.`nama`  ";
            }
        }else{ // Jika user tidak mengklik tombol tampilkan
  ...
  <thead>
    ...
    if($row2 > 0){ // Jika jumlah data lebih dari 0 (Berarti jika data ada)
        while($data2 = mysqli_fetch_array($sql2)){ // Ambil semua data dari hasil eksekusi $sql
            ...
            echo "<td>Rp. ".number_format($data2['harga'])."</td>";
            echo "<td>".$data2['qty']."</td>";
            echo "<td class='bg-primary'>Rp.".number_format($data2['total'])."</td>";
            echo "</tr>";
        }
    }else{ // Jika data tidak ada
        echo "<tr><td class='bg-danger' colspan='6'>Data Transaksi Tidak Ada</td></tr>";
